...
class _CategoryItemState extends State<CategoryItem> {
  ...
  void _loadProducts() async {
    List<Product> productsByCategory =
        await Category.getProductsByCategory(widget.CategoryId);
    setState(() {
                  ...
                  Container(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (_product.length % 2 != 0 &&
                            index == (_product.length / 2).ceil() - 1) {
                          return Row(children: [
                            ProductSuggestItem(
                              key: ValueKey<String>(_product[index * 2].id),
                              ProductsuggestReference: FirebaseDatabase.instance
                                  .ref()
                                  .child('products')
                                  .child(_product[index * 2].id.toString()),
                            ),
                          ]);
                        } else {
                          return Row(
                            children: [
                              ProductSuggestItem(
                                key: ValueKey<String>(_product[index * 2].id),
                                ProductsuggestReference: FirebaseDatabase
                                    .instance
                                    .ref()
                                    .child('products')
                                    .child(_product[index * 2].id.toString()),
                              ...
                              ProductSuggestItem(
                                key: ValueKey<String>(
                                    _product[index * 2 + 1].id),
                                ProductsuggestReference: FirebaseDatabase
                                    .instance
                                    .ref()
                                    .child('products')
                                    .child(
                                        _product[index * 2 + 1].id.toString()),
                              ),
                            ],
                          );
                        }
