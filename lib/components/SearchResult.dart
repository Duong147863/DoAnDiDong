...
class _SearchResultState extends State<SearchResult> {
  ...
  List<Product> filterProducts(String searchText) {
    ...
    for (Product product in allProducts) {
      ...
      if (productNameNormalized
          .toLowerCase()
          .contains(searchTermNormalized.toLowerCase())) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          'Kết quả tìm kiếm cho "${widget.searchText}"',
                  ...
                  Container(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (searchResults.length % 2 != 0 &&
                            index == (searchResults.length / 2).ceil() - 1) {
                          return Row(
                            children: [
                              ProductResultItem(
                                key: ValueKey<String>(
                                    searchResults[index * 2].id),
                                ProductReference: FirebaseDatabase.instance
                                    .ref()
                                    .child('products')
                                    .child(
                                        searchResults[index * 2].id.toString()),
                              ),
                            ],
                          );
                        } else {
                          return Row(
                            children: [
                              ...
                              ProductResultItem(
                                key: ValueKey<String>(
                                    searchResults[index * 2 + 1].id),
                                ProductReference: FirebaseDatabase.instance
                                    .ref()
                                    .child('products')
                                    .child(searchResults[index * 2 + 1]
                                        .id
                                        .toString()),
                              ),
                            ],
